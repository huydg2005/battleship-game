...
export class Prepare implements OnInit {
  ...
  constructor(
    ...
    this.route.queryParams.subscribe(params => {
      ...
      const paramPlayerId = params['playerId'] as 'player1' | 'player2' | undefined;
      const paramName = params['name'] || '';

      // Nếu đã truyền playerId từ waitroom → ưu tiên dùng
      if (paramPlayerId) {
        this.playerId = paramPlayerId;
        this.playerName = paramName;
        localStorage.setItem('playerId', this.playerId);
        localStorage.setItem('playerName', this.playerName);
      } else {
        // Nếu không có param, thử lấy từ localStorage
        const storedId = localStorage.getItem('playerId') as 'player1' | 'player2' | null;
      ...
      onAuthStateChanged(this.auth, async user => {
        ...
        // Nếu chưa biết playerId thì joinRoom để xác định
        if (!this.playerId) {
          await this.joinRoom();
        } else {
