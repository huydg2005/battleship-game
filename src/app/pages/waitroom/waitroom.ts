...
export class WaitRoom implements OnInit {
  ...
  ngOnInit() {
    ...
    this.currentPlayerId = localStorage.getItem('playerId') || ''; // lưu id khi login
  ...
  listenToRoom() {
    ...
    this.room$.subscribe((data: any) => {
      ...
      if (data.players && typeof data.players === 'object') {
        ...
        // Xác định id player hiện tại
        const found = this.players.find(p => p.name === this.currentPlayer);
        if (found) {
